...
public class ShipmentProcessTrigger : OneOfBase<BaseShipmentProcessEvent, BaseShipmentEvent, BaseCollectionBookingEvent>
{
    public static ShipmentProcessTrigger FromEvent(BaseShipmentWithProcessCategoryEvent trigger) =>
        trigger switch
        {
            ...
            BaseShipmentEvent manifestationAndDocumentsSubprocessEvent => new(manifestationAndDocumentsSubprocessEvent),
            BaseCollectionBookingEvent collectionBookingSubprocessEvent => new(collectionBookingSubprocessEvent),
            _ => throw new TriggerNotSupportedException(trigger.GetType().FullName),
        };

    private ShipmentProcessTrigger(OneOf<BaseShipmentProcessEvent, BaseShipmentEvent, BaseCollectionBookingEvent> input)
        : base(input) { }
}

public class TriggerNotSupportedException(string eventType) : Exception($"Trigger event '{eventType}' is not supported");
