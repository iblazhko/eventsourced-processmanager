    ...
    > RegisteredProcesses =
        new()
        {
            ...
            { DomesticShipmentProcessV1.ShipmentProcessCategory, new DomesticShipmentProcessV1() },
            {
                ...
                new InternationalShipmentProcessV1()
            },
        };

    public IShipmentProcess GetByCategory(ShipmentProcessCategory processCategory) =>
        RegisteredProcesses.TryGetValue(processCategory, out var process)
            ? process
            : throw new ProcessNotSupportedException(processCategory);
}

public class ProcessNotSupportedException(ShipmentProcessCategory processCategory)
    : Exception($"Process '{processCategory}' is not supported");
