...
public class CollectionBookingTriggersConsumer(
    ICollectionBookingSubprocess collectionBookingSubprocess
)
...
{
    public Task Consume(ConsumeContext<CreateCollectionBooking> context) =>
        collectionBookingSubprocess.Handle(context.Message);

    public Task Consume(ConsumeContext<ScheduleCollectionBooking> context) =>
        collectionBookingSubprocess.Handle(context.Message);

    public Task Consume(ConsumeContext<BookCollectionWithCarrier> context) =>
        collectionBookingSubprocess.Handle(context.Message);

    public Task Consume(
        ConsumeContext<CarrierIntegrationEvents.CollectionBookedWithCarrier> context
    ) => collectionBookingSubprocess.Handle(context.Message);

    public Task Consume(
        ConsumeContext<CarrierIntegrationEvents.CarrierCollectionBookingFailed> context
    ) => collectionBookingSubprocess.Handle(context.Message);
}
