    ...
    : IEventStreamProjection<ManifestationAndDocumentsState, BaseShipmentEvent>
{
    ...
    public ManifestationAndDocumentsState Apply(
        ...
        BaseShipmentEvent evt
    ) =>
        evt switch
        {
            ShipmentInitialized x
                => new(
            ...
            ShipmentLegManifested x
                => state with
                {
                    ManifestedLegs = state
                        ...
                        .Append(
                            state
                                .Legs.Where(l => l.CarrierId.Id == x.CarrierId)
                                .Select(l => l.ToManifestedLeg(x.TrackingNumber))
                                .Single()
                        )
                        .ToArray()
                },
            ...
            ShipmentLabelsGenerated x
                => state with
                {
                    Documents = state.Documents with { Labels = x.DocumentLocation }
                },
            CustomsInvoiceGenerated x
                => state with
                {
                    Documents = state.Documents with { Invoice = x.DocumentLocation }
                },
            ShipmentReceiptGenerated x
                => state with
                {
                    Documents = state.Documents with { Receipt = x.DocumentLocation }
                },
            ShipmentCombinedDocumentGenerated x
                => state with
                {
                    Documents = state.Documents with { CombinedDocument = x.DocumentLocation }
                },
            _ => state
        };
}
